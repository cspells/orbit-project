...
function handleControlChange(newValues) {
  OrbitalElements.SemiMajorAxis = newValues.semiMajorAxis;
  OrbitalElements.Eccentricity = newValues.eccentricity;
  OrbitalElements.Inclination = newValues.inclination;
  OrbitalElements.RAAN = newValues.RAAN;
  OrbitalElements.ArgumentPerigee = newValues.argumentPerigee;
  OrbitalElements.TrueAnomaly = newValues.trueAnomaly;
  updateOrbit(OrbitalElements, simulationTime);
}

const root = ReactDOM.createRoot(document.querySelector('#gui_container'));
root.render(<App onChange={handleControlChange} />);
