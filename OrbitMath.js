import { matrix, multiply } from 'mathjs'; //'https://cdn.skypack.dev/mathjs';
import { Vector3 } from 'three'; //'https://cdn.skypack.dev/three@0.136.0';
...
export function COE2IJK(OrbitalElementsIn, computeFullOrbit = false, trueAnomalyRadians=null, rotateForCamera=false) 
{
    ...
    const alphaPQW2IJK = matrix(
        [[c_omega * c_Omega - s_omega * c_i * s_Omega, -s_omega * c_Omega - c_omega * c_i * s_Omega, s_i * s_Omega],
        [c_omega * s_Omega + s_omega * c_i * c_Omega, -s_omega * s_Omega + c_omega * c_i * c_Omega, -s_i * c_Omega],
        [s_omega * s_i, c_omega * s_i, c_i]]);

    function computePosition(thetaRadians) 
    {
        var r = P / (1 + e * Math.cos(thetaRadians));
        var rplane = matrix([r * Math.cos(thetaRadians), r * Math.sin(thetaRadians), 0]);
    ...
    {
        const points = []          
        for (var theta = 0; theta < TWO_PI; theta += PI / 100) 
        {
...
{
    return TWO_PI * Math.sqrt( ( Math.pow(OrbitalElementsIn.SemiMajorAxis, 3) ) / mue );
}
...
export function propagate(OrbitalElementsIn, deltaTime, thetaRadians) 
{
    ...
    const n = ( TWO_PI ) / P; 
    // Eccentric Anomaly
    var E = Math.acos((e+Math.cos(thetaRadians))/(1+e*Math.cos(thetaRadians)));
    
    E = mod(E, TWO_PI);
    if ( mod(thetaRadians, TWO_PI ) > PI)
    {
        E=TWO_PI-E;
    }
...
{
    // startTime = 0
    const GWOff = 0*EARTH_ROTATION_RATE * startTime;
    const geodetic = []
    const P = computePeriod(OrbitalElementsIn);
